...
void loop()
{
  ...
  checkDistance1(distance1, tone1, 1500, &t1);
  checkDistance2(distance2, tone2, 1250, &t2);
  btout(distance1,distance2);
...
void checkDistance1(long distance, Tone toneobj, int frequency, long *timer){
...
void checkDistance2(long distance, Tone toneobj, int frequency, long *timer){
  if(distance<5.9&&distance>0){
    if(millis()-*timer > long(5.4*distance-81)||distance<15){
      *timer=millis();
      toneobj.play(frequency,100);
    }
